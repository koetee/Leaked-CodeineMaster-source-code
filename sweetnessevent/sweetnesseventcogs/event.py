...
class GiveEventPrizeDropdown(disnake.ui.Select):
    def __init__(self, bot, members):
        ...
        for member_id in members:
            try:
                member = disnake.utils.get(self.bot.get_guild(1001139842131955752).members, id = int(member_id))
...
class ActionView(disnake.ui.View):
    def __init__(self, member):
        super().__init__()
        checks = [
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, closemod], 'label': 'Профиль', 'custom_id': "profile_action", 'emoji': f'{files.find_one({"_id": "user"})["emoji_take"]}', 'row': 0},
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, closemod], 'label': 'Топы', 'custom_id': "places_action", 'emoji': f'{files.find_one({"_id": "trophy"})["emoji_take"]}', 'row': 0},
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, closemod], 'label': 'Отпуск', 'custom_id': "rest_action", 'emoji': f'{files.find_one({"_id": "rest"})["emoji_take"]}', 'row': 0},
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, closemod], 'label': 'Выговор', 'custom_id': "warn_staff_action", 'emoji': f'{files.find_one({"_id": "staff_warn"})["emoji_take"]}', 'row': 1},
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, closemod], 'label': 'Выговоры', 'custom_id': "warns_action", 'emoji': f'{files.find_one({"_id": "list"})["emoji_take"]}', 'row': 1},
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496], 'label': 'Настроить', 'custom_id': "settings_action", 'emoji': f'{files.find_one({"_id": "basket"})["emoji_take"]}', 'row': 1},
            {'roles': [890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, closemod], 'label': 'Отмена', 'custom_id': "exit_action", 'emoji': f'{files.find_one({"_id": "basket"})["emoji_take"]}', 'row': 1},
        ]
...
class eventbot(commands.Cog):
    ...
    @commands.has_any_role(890311730939433011, 1143564838916870180, 1131539393207865354, 1142875756209381496, 890311730939433011)
    ...
    async def on_modal_submit(self, inter):
        ...
        if custom_id.endswith("event"):
            if custom_id == "money_event":
                ...
                embed.add_field(name = "Число валюты", value = f"{количество}")
                msg = await self.bot.get_channel(1123711704329629728).send('<@1129862664953282722>', embed = embed, view = YesOrno())
